...
async function energyCollect() {
  ...
  if (response.resultCode === '0') {
    message += `【物品名称】“${$.petInfo.goodsInfo.goodsName}”\n`;
    message += `【已获勋章】${response.result.medalNum}块，还需收集${response.result.needCollectMedalNum}块\n`;
    message += `【完成进度】第${response.result.medalNum + 1}块勋章${response.result.medalPercent}%，还需收集${response.result.needCollectEnergy}好感\n`;
  }
}
//再次投食
