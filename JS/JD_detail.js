...
function notify() {
  return new Promise(resolve => {
    try {
      ...
      var five = `【其他总计】:  ${Subsidy+Money+Steel}${Subsidy||Money||Steel?`\n`:`获取失败\n`}`
      ...
      $nobyda.notify("", "", Name + two + three + four + five + notify, {
        'media-url': $nobyda.headUrl || 'https://cdn.jsdelivr.net/gh/NobyDa/mini@master/Color/jd.png'
      });
      $nobyda.headUrl = null;
      if ($nobyda.isJSBox) {
        Shortcut = (typeof(Shortcut) == 'undefined' ? '' : Shortcut) + Name + two + three + four + five + "\n"
      }
      double();
    } catch (eor) {
